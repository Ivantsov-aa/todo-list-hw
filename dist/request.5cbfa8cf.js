...
function getTaskApi() {
  ...
  return getTask.then(function (response) {
...
function createTaskApi() {
  tasks.forEach(function (task) {
    var createTask = axios.post('http://localhost:3000/todos', task);
    return createTask.then(function (response) {
...
function deleteTaskApi(id) {
  var deleteTask = axios.delete("http://localhost:3000/todos/".concat(id));
  return deleteTask.then(function (response) {
    return response.data;
  });
}
...
function updateTaskApi(id) {
  tasks.forEach(function (task) {
    var updateTask = axios.put("http://localhost:3000/todos/".concat(id), task);
    return updateTask.then(function (response) {
      return response.data;
    });
  });
}
...
function dateChange() {
  ...
  return dateApi.then(function (response) {
    return response.data;
  });
}

function compareDate() {
  var dateApi = axios.get('http://localhost:3000/info');
  return dateApi.then(function (response) {
    if (dateChangeLS > response.data.lastDateModified) {
      dateChange();
      createTaskApi(); // updateTaskApi(task.id);
    }
  });
}
